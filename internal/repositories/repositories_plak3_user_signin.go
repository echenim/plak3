...
func (r *Plak3UserSignInRepository) SignIn(u views.Plak3Login) (views.Plak3LoginUser, error) {
	...
	query := "SELECT user_id, name, username, email,  passwordhash, email_confirmed, phone_number_confirmed, mfa_enabled, lockedout,roles FROM  view_plak_user_signed_in WHERE username = $1"
	...
	if err := row.Scan(&login_user_info.Id, &login_user_info.Name, &login_user_info.UserName, &login_user_info.Email, &login_user_info.PasswordHash, &login_user_info.EmailConfirmed,
		&login_user_info.PhoneNumberConfirmed, &login_user_info.MfaEnabled, &login_user_info.LockedOut, &rolesJSON); err != nil {
